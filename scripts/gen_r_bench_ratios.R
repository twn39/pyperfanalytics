...
# Load data
df <- read.csv("data/test_data_v2.csv", row.names=1)
# Convert to xts
data_xts <- as.xts(df, order.by=as.Date(rownames(df)))

# Assets: QQQ, IWM, EEM
# Benchmark for CAPM: QQQ (or use SPY if available, but here QQQ is likely the benchmark of interest for v2)
# Using QQQ as the "market" for internal attribution tests in v2
Ra <- data_xts[, c("QQQ", "IWM", "EEM")]
Rb <- data_xts[, "QQQ"]
Rf <- data_xts[, "BIL"]
...
# Save to JSON
write_json(metrics, "data/r_benchmarks_v2.json", auto_unbox=TRUE, pretty=TRUE, digits=NA)
cat("Benchmarks saved to data/r_benchmarks_v2.json\n")
