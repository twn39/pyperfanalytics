...
tasks = [
    ...
    (
        ...
        lambda: pa.chart_relative_performance(data[["GLD"]], data[["SPY"]]),
        "chart.RelativePerformance(sub_data[,1], sub_data[,2])",
    ),
    (
        "capture_ratios",
        lambda: pa.chart_capture_ratios(df[["GLD", "TLT"]], df[["SPY"]]),
        "chart.CaptureRatios(xts_data[,c('GLD','TLT')], xts_data[,'SPY'])",
    ),
    (
        "rolling_performance",
        lambda: pa.chart_rolling_performance(data, width=12),
        "chart.RollingPerformance(sub_data, width=12)",
    ),
    (
        "bar_var_gld",
        lambda: pa.chart_bar_var(data[["GLD"]], width=12),
        "chart.BarVaR(sub_data[,1,drop=FALSE], width=12)",
    ),
    (
        "var_sensitivity_gld",
        lambda: pa.chart_var_sensitivity(data[["GLD"]]),
        "chart.VaRSensitivity(sub_data[,1,drop=FALSE])",
    ),
    (
        "rolling_regression_beta",
        lambda: pa.chart_rolling_regression(data[["GLD"]], data[["SPY"]], width=12, attribute="Beta"),
        "chart.RollingRegression(sub_data[,1], sub_data[,2], width=12, attribute='Beta')",
    ),
    (
        "charts_rolling_regression",
        lambda: pa.charts_rolling_regression(data[["GLD"]], data[["SPY"]], width=12),
        "charts.RollingRegression(sub_data[,1], sub_data[,2], width=12)",
    ),
    (
        "rolling_correlation",
        lambda: pa.chart_rolling_correlation(data[["GLD"]], data[["SPY"]], width=12),
        "chart.RollingCorrelation(sub_data[,1], sub_data[,2], width=12)",
    ),
    (
        "risk_return_scatter",
        lambda: pa.chart_risk_return_scatter(df, Rf=0.01 / 12),
        "chart.RiskReturnScatter(xts_data, Rf=0.01/12)",
    ),
    (
        "histogram",
        lambda: pa.chart_histogram(data[["GLD"]]),
        "chart.Histogram(sub_data[,1], methods=c('add.density', 'add.normal', 'add.risk'))",
    ),
    ...
    (
        "events",
        lambda: pa.chart_events(data[["GLD"]], dates=event_dates, prior=12, post=12),
        "chart.Events(sub_data[,1,drop=FALSE], dates=c('2020-03-31', '2022-01-31'), prior=12, post=12)",
    ),
    (
        "snail_trail",
        lambda: pa.chart_snail_trail(data, width=12, stepsize=3),
        "chart.SnailTrail(sub_data, width=12, stepsize=3)",
    ),
